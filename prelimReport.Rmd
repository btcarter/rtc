...
df.pp <- df.processed %>% 
  filter(
    rtc_diet_groups == "Preparation" &
      rtc_exercise_groups == "Preparation"
  ) %>% 
  sample_frac(
    size = 0.2
  )

df.pp.joined <- df.processed %>% 
  mutate(
    filterCol = ifelse(
      rtc_diet_groups == "Preparation" &
        rtc_exercise_groups == "Preparation",
...
mod_test <- glm(
  df.pp.joined[["Lost 7 percent during core"]] ~ rtc_diet_groups,
  data = df.pp.joined,
  family = "binomial"
)
