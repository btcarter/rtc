...
Statistical models were created in an attempt to predict outcomes based on patient demographics and baseline response to the readiness to change survey questions. Two general models were used to predict success in the program - a simple model incorporating only the answers on the readiness to change survey and a complex model incorporating both an interaction between the answers on the survey and the effects of age, gender, and BMI. These models were employed in a logistic fashion when predicting nominal outcomes (e.g. did they lose 7% of their body weight during the program) and in a linear model when predicting a continuous outcome (e.g. how much of their body weight did they lose during the program).

These predictive models were evaluated for each of the outcome measures. The results are organized by model type (logistic or linear, simple or complex) and outcome variable predicted by the model. $\beta$ estimates and *p*-values are presented in tables. Logistic model performance is quantified using *Akaike information criteria* (AIC). Linear model performance is quantified using an adjusted-R$^2$.
...
This model is called the simple model because it assumes no interactions and only examines the effect of response to the readiness to change survey on the outcome variables without considering interactions between the survey responses or the effect of other factors such as age.
...
model_log <- function(x){
  ...
  p.value <- logit_p(model)

  b <- list("model" = model, "p" = p.value)
...
log_simple <- lapply(binary_vars, model_log)

```

##### Lost 7 percent during core
```{r}
mod <- "Lost 7 percent during core"

a <- as.data.frame(
  summary(log_simple[[mod]]$model)$coefficients
)

aic <- log_simple[[mod]]$model$aic
```
...
```{r}
mod <- "Lost 5 percent during core"
...
```{r}
mod <- "Met exercise goal at least 50 percent"
...
The complex model examines the effect of readiness to change and also includes an interaction between responses on the readiness to change survey, as well as potential effects of age, gender and BMI.
...
log_complex <- lapply(binary_vars, model_log)
...
a <- as.data.frame(
  summary(log_complex[[mod]]$model)$coefficients
)

aic <- log_complex[[mod]]$model$aic
```
...
Overall, the models performed very poorly. The best logistic model had an AIC of `r log_simple[["Lost 7 percent during core"]]$aic` to predict whether an individual lost 7 percent of their body weight during the core sessions. This is quite high, ideally 
...
knitr::kable(
  performance,
  caption = "Logistic model performance when predicting outcome variables."
) %>% 
...
df.processed <- df.processed %>% 
  mutate(
    ...
    rtc_exercise_post_groups = ifelse(
...
model_log <- function(x){
  ...
  model <- glm(
    formula = 
      df.processed[[x]] ~ 
      rtc_diet_post_groups * 
      rtc_exercise_post_groups +
      Age + 
      Gender +
      BMI, 
    data = df.processed, 
